 ...
 * @author Kervin
 */
@WebServlet(name = "ActualizarCompania", urlPatterns = {"/ActualizarCompania"})
public class ActualizarCompania extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        PrintWriter p = response.getWriter();Gson gson = builder.create();
        try {
            int companyid = Integer.parseInt(request.getParameter("id"));
            String companyname = request.getParameter("companyname");
            String neighborhood = request.getParameter("neighborhood");
            String zipcode = request.getParameter("zipcode");
            String city = request.getParameter("city");String country = request.getParameter("country");
            ...
            if (companyname == null) {
                ...
                p.write(ejbC.UpdateRol(companyid, companyname, neighborhood, zipcode,
                        city, country, state, region, street, streetnumber,
                        phone, RFC, logo));
            }
        } catch (NumberFormatException ex) {
            msn.setCode(HttpServletResponse.SC_BAD_REQUEST);
            msn.setMessage("El identificador de la compañia es obligatoria");
            msn.setDetail(ex.getMessage());
            p.print(gson.toJson(msn));
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
